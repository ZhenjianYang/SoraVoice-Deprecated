...
RCImpl::RCImpl(const char * name) {
	...
	if (!data) {
		...
		if (it != _rcTable.end()) {
			...
			HRSRC hrsrc = FindResourceA(_hs, MAKEINTRESOURCE(idtype.first), idtype.second.c_str());
			if (hrsrc) {
				HGLOBAL h = LoadResource(_hs, hrsrc);
				if (h) {
					char* first = (char*)LockResource(h);
					if (first) {
						size = std::min(MaxRCSize, (unsigned)SizeofResource(_hs, hrsrc));
