...
%nasm% -i%dir_asm%\ -o %dir_bin%\%%i_d3d -Dsora_%%i %dir_asm%\asm_d3d.asm

Importer_EXE %dir_new%\%%i_voice.exe  %dir_main%\%%i_main.exe -r zrep_%%i.txt -m %dir_asm%\macro_%%i -b %dir_bin%\%%i_text %dir_bin%\%%i_dududu %dir_bin%\%%i_dlgse %dir_bin%\%%i_input %dir_bin%\%%i_d3d
)